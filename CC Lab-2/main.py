...
def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if not user:
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "error": "❌ Invalid username or password", "user": "PES2UG23AM033"}
        )
...
def checkout(request: Request):
    ...
    return templates.TemplateResponse(
        "checkout.html",
        {"request": request, "total": total, "user": "PES2UG23AM033"}
    )
@app.exception_handler(Exception)
async def global_exception_handler(request: Request, exc: Exception):
    # Try to keep user on UI even when it crashes
    user = request.query_params.get("user", "PES2UG23AM033")
